...
export type ThemeDataActionType = 
    | { type: 'setIsLight', isLight: boolean }
...
export const ThemeDataContext = createContext<null|ThemeDataContextType>(null)

const useThemeData = () => {
    return useContext(ThemeDataContext) as ThemeDataContextType
}

export const ThemeDataReducer = (state: ThemeDataType, action: ThemeDataActionType) => {
    if (action) null;
    return state;
}
